...
public class SmelterScreen extends HandledScreen<SmelterScreenHandler> {
    ...
    private static final int FLUID_X = 81;
    private static final int FLUID_Y = 19;
    private static final int FLUID_W = 48;
    private static final int FLUID_H = 48;

    private static final int TEMP_BAR_X = 78;
    private static final int TEMP_BAR_Y = 19;
    private static final int TEMP_BAR_W = 2;
    private static final int TEMP_BAR_H = 48;
    private static final int TEMP_U = 176;
    private static final int TEMP_V = 14;

    private static final int SMELT_BAR_X = 14;
    private static final int SMELT_BAR_W = 2;
    private static final int SMELT_BAR_H = 16;
    private static final int SMELT_U = 178;
    private static final int SMELT_V = 14;
    private static final int[] SMELT_SLOT_Y = {17, 35, 53};

    private static final int FLAME_X = 153;
    ...
    private void drawFluidFill(DrawContext context, int x, int y) {
        ...
        Identifier spriteId = MoltenHelper.getMoltenSpriteId(handler.getMetalType());
        Sprite sprite = MinecraftClient.getInstance().getSpriteAtlas(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE).apply(spriteId);

        int color = MoltenHelper.getMoltenColor(handler.getMetalType());
        ...
        for (int tileX = 0; tileX < FLUID_W; tileX += spriteW) {
            for (int tileY = 0; tileY < filledH; tileY += spriteH) {
                int drawW = Math.min(spriteW, FLUID_W - tileX);
                int drawH = Math.min(spriteH, filledH - tileY);
    ...
    /**
     * Draws a vertical bar that fills from the bottom upward.
     *
     * @param screenX top-left screen X of the bar area
     * @param screenY top-left screen Y of the bar area
     * @param barW    pixel width  of the bar
     * @param barH    full pixel height of the bar
     * @param u       texture U of the fill graphic (full bar, same size as barW × barH)
     * @param v       texture V of the fill graphic
    ...
    private void drawFuelFlame(DrawContext context, int x, int y) {
        ...
        float fuelPercent = handler.getFuelPercent();
        int litH = MathHelper.ceil(FLAME_H * fuelPercent);
        if (litH <= 0) return;
    ...
    private void drawBurnArrow(DrawContext context, int x, int y) {
        float progress = handler.getSmeltPercent(0);
        int arrowW = MathHelper.ceil(ARROW_W * progress);
        if (arrowW <= 0) return;
    ...
    private void drawFluidTooltip(DrawContext context, int relX, int relY) {
        if (relX >= FLUID_X && relX <= FLUID_X + FLUID_W && relY >= FLUID_Y && relY <= FLUID_Y + FLUID_H) {
            ...
            String name = MoltenHelper.getMetalName(handler.getMetalType());
            context.drawTooltip(textRenderer, Text.literal(name + ": " + mb + " / " + cap + " mB"), relX, relY);
        }
    }

    private void drawTemperatureTooltip(DrawContext context, int relX, int relY) {
        if (relX >= TEMP_BAR_X - 1 && relX <= TEMP_BAR_X - 1 + TEMP_BAR_W && relY >= TEMP_BAR_Y && relY <= TEMP_BAR_Y + TEMP_BAR_H) {

            int temp = handler.getTemperature();
            int maxTemp = handler.getMaxTemperature();
            context.drawTooltip(textRenderer, Text.literal("Temperature: " + temp + " / " + maxTemp + " °C"), relX, relY);
        }
    }
    ...
    private void drawSmeltingTooltip(DrawContext context, int relX, int relY) {
        for (int i = 0; i < 3; i++) {
            ...
            if (relX >= SMELT_BAR_X && relX <= SMELT_BAR_X + SMELT_BAR_W
                    && relY >= barY && relY <= barY + SMELT_BAR_H) {

                int progress = handler.getSmeltProgress(i);
                int total = handler.getSmeltTotal(i);
                if (total > 0) {
                    int percent = (int) (handler.getSmeltPercent(i) * 100);
                    context.drawTooltip(textRenderer, Text.literal("Smelting: " + percent + "% (" + progress + "/" + total + " ticks)"), relX, relY);
                }
                break;
            }
        }
